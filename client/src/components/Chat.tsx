...
export function Chat({ messages, onSendMessage, myPlayerId }: ChatProps) {
  ...
  return (
    <div className="fixed top-20 right-4 sm:right-8 z-[100]">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ height: 0, opacity: 0, scale: 0.95 }}
            animate={{ height: "calc(100vh - 40px)", maxHeight: "600px", opacity: 1, scale: 1 }}
            exit={{ height: 0, opacity: 0, scale: 0.95 }}
            className="bg-white rounded-2xl shadow-2xl border border-border overflow-hidden flex flex-col overscroll-none touch-none w-[calc(100vw-32px)] sm:w-[400px]"
            style={{ 
              position: 'fixed', 
              top: '20px', 
              right: '16px',
          ...
          >
            ...
            <div className="p-3 sm:p-4 border-b bg-primary/5 flex justify-between items-center shrink-0">
              <Button
                ...
                className="rounded-lg hover:bg-black/5 text-muted-foreground transition-all h-8 w-8 sm:h-10 sm:w-10"
              >
      ...
      {!isOpen && (
        ...
        >
          <Button
            ...
            size="icon"
            className="rounded-full w-12 h-12 sm:w-14 sm:h-14 shadow-lg bg-primary hover:bg-primary/90 text-white btn-bounce relative"
          >
            <MessageSquare className="w-6 h-6 sm:w-7 sm:h-7" />
            {messages.length > 0 && (
              <span className="absolute top-0 right-0 w-3 h-3 bg-secondary rounded-full border-2 border-primary animate-pulse" />
            )}
          </Button>
        </motion.div>
      )}
